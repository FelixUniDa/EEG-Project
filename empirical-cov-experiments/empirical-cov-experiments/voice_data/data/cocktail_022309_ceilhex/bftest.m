...
%  test/development of beamforming and sources separation algorithms.
...
%  the party of multiple simultaneous conversations. The microphone
...
%  there are weights applied to each microphone channel base on thier
...
%  The window size WS can also be changed.
%   Outputs
...
figure
   ...
   pause(tsig(end)+1)
    ...
    [sii_cm,tax] = intel(sigclosemic,nosclosemic,fs,100e-3);
    sii_m = mean(sii_cm);
    sii_s = std(sii_cm);
    ...
    [siib,taxb] = intel(sig_beam,nos_beam,fs,100e-3);
    siib_m = mean(siib);
    siib_s = std(siib);
