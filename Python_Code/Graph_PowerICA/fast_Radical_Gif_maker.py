...
def RADICAL(X,K=300,sweeps=5, seed=None):
    ...
    stdev=0.175
    # ************************************************************
    for filename in glob.glob("results_gif/ICA_*"):
            os.remove(filename) 
    ...
    for sweepNum in range(0, sweeps):
        ...
        for i in range(0, dim-1):
            for j in range(i+1, dim):
                temp_data = totalRot @ X
                sns.set_style("darkgrid")
                fig1, axs1 = plt.subplots(4,figsize = (12,8), sharex=True)
                ...
                while(v<4):
                    # input signals
                    axs1[v].plot(temp_data .T[:, v], lw=1)
                    v +=1 
                # generate gif
                plt.close()

                # save image
                fig1.savefig("results_gif/ICA_{}.jpeg".format(counter))
                counter += 1
        ...
        # while(v<4):
        #     # input signals
        #     axs1[i].plot(temp_data.T[:, i], lw=1)
        #     fig1.suptitle('ICA')
        #     v +=1 
        # # generate gif
        # plt.close()
        ...
        # # save image
        # fig1.savefig("results/ICA_{}.jpeg".format(counter), bbox_inches='tight')
        # print("und hier auch")

    filenames = ["results_gif/ICA_{}.jpeg".format(i) for i in range(0,counter)]
    generate_gif(filenames, gifname="results_gif/ICA.gif")
