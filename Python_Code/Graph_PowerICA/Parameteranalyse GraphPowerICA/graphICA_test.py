...
if __name__ == "__main__":
  nonlins = ['arctan','pow3','tanh','gaus']
  #nonlins = ['gaus']
  bs = [0.2,0.1]  
  Signaltypes= ["GMA","Standard"]
  for b in bs:
    runs = 100
      ...
      for Signaltype in Signaltypes:
        ...
        #               create_signal(f=25,c='sawt',x=1000)]).T
          ...
          if Signaltype == "GMA" :
            ...
            A1 = genrAs(N)
            A2 = A1@A1
            Ws = np.array([A1,A1,A1,A1,A2,A2,A2,A2]).reshape(1000,1000,8)
          ...
          W_graphpower,_ = Graph_powerICA(white_data,nonlin=nonlin,Ws=Ws,b=b)
