...
if __name__ == "__main__":
  ...
  for i in range(0,runs):
    ...
    mixdata_noise = np.stack([create_outlier(apply_noise(dat, type='white', SNR_dB=100),
                                             std=100, prop=0.01, type='impulse') for dat in mixeddata])
    ...
    white_data, W_whiten, W_dewhiten,_ = whitening(mixdata_noise_ms, type='sample')

    plt.plot(white_data.T)
    plt.show()

    C = np.triu(np.corrcoef(white_data.T), k=1) #Mscat(mixeddata, 'Huber')
    thres = 0.2
    ...
    W_graphpower,_ = Graph_powerICA(white_data, 'gaus', Ws1, b=0.65)
