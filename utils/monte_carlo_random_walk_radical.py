    ...
    for i in range(0, n_runs):
        ...
        if(new_MM):
            ...
            if data_type == 'standard':
                ...
                mixdata_noise = np.stack([create_outlier(apply_noise(dat, type='white', SNR_dB=noise_lvl), prop=p_outlier, type=outlier_type, std=std_outlier) for dat in mixdata])
                noise = mixdata_noise - mixdata
                # plt.plot(mixdata[0, :].T)
                # plt.show()
                # plt.plot(mixdata_noise[0, :].T)
                # plt.show()
            ...
            if data_type == 'delorme':
                ...
                for i1 in range(0, eeg_components):
                    if (i1 < artifacts):
                        data_outl, outlier = add_artifact(eeg_data[0::, i1], fs, prop=prop, snr_dB=snr_dB,
                                                              number=artifacts, type=type[i1], seed=None)
                        eeg_data_artif[0::, i1] = data_outl
                        eeg_data_outlier[0::, i1] = outlier
                    else:
                        eeg_data_artif[0::, i1] = eeg_data[0::, i1]
                        eeg_data_outlier[0::, i1] = eeg_data_outlier[0::, i1]
                ...
                data_delorme = np.concatenate([data, eeg_data_outlier], axis=1)
            ...
            if(data_type == 'delorme'):
                MSE_data[i] = np.mean(MSE(W @ white_data - W @ W_whiten @ noise, data_delorme.T))
                SNR_data[i] = np.mean(SNR(W @ white_data - W @ W_whiten @ noise, data_delorme.T))
            else:
...
if __name__ == "__main__":
    ...
    type_list_to_test = ["Type 6", "Type 7"]  #"Type 1", "Type 2", "Type 3", "Type 4", "Type 5",  "Type 1", "Type 2", "Type 3", "Type 4"
