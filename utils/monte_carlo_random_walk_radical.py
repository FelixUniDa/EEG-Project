...
def monte_carlo_run(n_runs, data_size, ica_method, data_type='standard', seed=None, noise_lvl=20, std_outlier=3,
    ...
    df.to_csv(os.path.join(BASE_DIR, 'utils', 'results_Monte_Carlo_RADICAL', 'Monte_Carlo_runs_RADICAL.csv'),
              index=True,
              header=True, mode='a')
...
if __name__ == "__main__":
    ...
    type_dict.update({"Type 8": [1000, 0.000, n_runs, 10000, 'impulse',
                                 std_outlier]})  # (dB Noise, outlier proportion, runs, sample_size)
    ...
    for name in type_list_to_test:
        ...
        if name == "Type 1" or name == "Type 2" or name == "Type 3" or name == "Type 4":
            ...
            for s in sample_size:
                ...
                mds, median_MSEs, median_SNRs, mean_MSEs, mean_SNRs, hSNRs = monte_carlo_run(n_runs, s, ica_method,
                                                                                             seed=None,
                ...
                d_mean_SNR = {'Location': 'mean', 'Signal to Noise Ratio\n (Mixed Signals) in dB': mean_SNRs,
                              'Sample Size': np.repeat(s, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}
                d_mean_MSE = {'Location': 'mean', 'Mean Squared Error': mean_MSEs, 'Sample Size': np.repeat(s, n_runs),
                              '# MC Runs': np.repeat(n_runs, n_runs)}

                d_median_SNR = {'Location': 'median', 'Signal to Noise Ratio\n (Mixed Signals) in dB': median_SNRs,
                                'Sample Size': np.repeat(s, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}
                d_median_MSE = {'Location': 'median', 'Mean Squared Error': median_MSEs,
                                'Sample Size': np.repeat(s, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}

                d_md = {'Minimum Distance': mds, 'Sample Size': np.repeat(s, n_runs),
                        '# MC Runs': np.repeat(n_runs, n_runs)}
            ...
            sns.boxplot(ax=axes[1], x='Sample Size', y='Mean Squared Error', hue='Location', data=df_MSE).set_title(
                title_MSE)
            plt.ylim(top=60, bottom=0)
            sns.boxplot(ax=axes[2], x='Sample Size', y='Signal to Noise Ratio\n (Mixed Signals) in dB', hue='Location',
                        data=df_SNR).set_title(title_SNR)
        ...
        if name == "Type 5":
            ...
            for snr in noise:
                ...
                mds, median_MSEs, median_SNRs, mean_MSEs, mean_SNRs, hSNRs = monte_carlo_run(n_runs, s, ica_method,
                                                                                             seed=None,
                                                                                             data_type=signal_type,
                                                                                             noise_lvl=snr, p_outlier=p,
                                                                                             std_outlier=std_outlier,
                                                                                             outlier_type=outlier_type)

                d_mean_SNR = {'Location': 'mean', 'Signal to Noise Ratio\n (Mixed Signals) in dB': mean_SNRs,
                              'SNR(additive noise)': np.repeat(snr, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}
                d_mean_MSE = {'Location': 'mean', 'Mean Squared Error': mean_MSEs,
                              'SNR(additive noise)': np.repeat(snr, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}
                d_median_SNR = {'Location': 'median', 'Signal to Noise Ratio\n (Mixed Signals) in dB': median_SNRs,
                                'SNR(additive noise)': np.repeat(snr, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}
                d_median_MSE = {'Location': 'median', 'Mean Squared Error': median_MSEs,
                                'SNR(additive noise)': np.repeat(snr, n_runs), '# MC Runs': np.repeat(n_runs, n_runs)}
                d_md = {'Minimum Distance': mds, 'SNR(additive noise)': np.repeat(snr, n_runs),
                        '# MC Runs': np.repeat(n_runs, n_runs)}
                temp_md = pd.DataFrame(data=d_md)
                temp_mean_MSE = pd.DataFrame(data=d_mean_MSE)
                temp_mean_SNR = pd.DataFrame(data=d_mean_SNR)
            ...
            title_md = name + ':  ' + 'runs: ' + str(n_runs) + ', ' + str(noise[-1]) + "-" + str(
                noise[0]) + 'dB noise' + ', ' + str(p * 100) + ' % outliers'
            file_name_md = 'MD_' + ica_method + '_' + name + '_' + str(n_runs) + 'Runs' + '_' + str(
                noise) + 'dB_' + 'p_outliers_' + str(p) + '.jpg'

            title_MSE = name + ':  ' + 'runs: ' + str(n_runs) + ', ' + str(noise[-1]) + "-" + str(
                noise[0]) + 'dB noise' + ', ' + str(p * 100) + ' % outliers'
            file_name_MSE = 'MSE_' + ica_method + '_' + name + '_' + str(n_runs) + 'Runs' + '_' + str(
                noise) + 'dB_' + 'p_outliers_' + str(p) + '.jpg'

            title_SNR = name + ':  ' + 'runs: ' + str(n_runs) + ', ' + str(noise[-1]) + "-" + str(
                noise[0]) + 'dB noise' + ', ' + str(p * 100) + '% outliers'
            file_name_SNR = 'SNR_' + ica_method + '_' + name + '_' + str(n_runs) + 'Runs' + '_' + str(
                noise) + 'dB_' + 'p_outliers_' + str(p) + '.jpg'
            ...
            sns.boxplot(ax=axes[1], x='SNR(additive noise)', y='Mean Squared Error', hue='Location',
                        data=df_MSE).set_title(title_MSE)
            plt.ylim(top=60, bottom=0)
            sns.boxplot(ax=axes[2], x='SNR(additive noise)', y='Signal to Noise Ratio\n (Mixed Signals) in dB',
                        hue='Location', data=df_SNR).set_title(title_SNR)
            plt.savefig(os.path.join(folder_to_save, file_name), dpi=300)
            plt.show()
        ...
        if name == "Type 6" or name == "Type 7":
            ...
            for percentage in p:
                ...
                mds, median_MSEs, median_SNRs, mean_MSEs, mean_SNRs, hSNRs = monte_carlo_run(n_runs, s, ica_method,
                                                                                             ...
                                                                                             data_type=signal_type,
                                                                                             noise_lvl=noise,
                                                                                             p_outlier=percentage,
                                                                                             std_outlier=std_outlier,
                                                                                             outlier_type=outlier_type)

                d_mean_SNR = {'Location': 'mean', 'Signal to Noise Ratio\n (Mixed Signals) in dB': mean_SNRs,
                              'Outlier Percentage': np.repeat(percentage, n_runs),
                              '# MC Runs': np.repeat(n_runs, n_runs)}
                d_mean_MSE = {'Location': 'mean', 'Mean Squared Error': mean_MSEs,
                              'Outlier Percentage': np.repeat(percentage, n_runs),
                              '# MC Runs': np.repeat(n_runs, n_runs)}
            ...
            sns.boxplot(ax=axes[1], x='Outlier Percentage', y='Mean Squared Error', hue='Location',
                        data=df_MSE).set_title(title_MSE)
        ...
        if name == "Type 8":
            ...
            for ps in partitions:
                ...
                # df = df.append(temp)
                mds, median_MSEs, median_SNRs, mean_MSEs, mean_SNRs, hSNRs = monte_carlo_run(n_runs, sample_size,
                                                                                             ica_method,
                                                                                             data_type=signal_type,
                                                                                             seed=None, noise_lvl=noise,
                                                                                             p_outlier=p,
                                                                                             outlier_type=outlier_type,
                                                                                             partition=(ps))
        ...
        if name == "Type scat":
            ...
            d_scatter = {'Median of SNRs (dB)': median_SNRs_ges, 'Mean of SNRs (dB)': mean_SNRs_ges,
                         'Median of MSEs': median_MSEs_ges, 'Mean of MSEs': mean_MSEs_ges, 'Minimum Distance': mds_ges,
                         'Theor. MD (from SNR)': theoretical_mds}
            ...
            sns.regplot(ax=axes[0, 0], data=df_scatter, y="Median of MSEs", x='Minimum Distance', order=2, ci=None,
                        x_jitter=.05, line_kws={"color": "red"})
            sns.regplot(ax=axes[0, 1], data=df_scatter, y='Median of SNRs (dB)', x='Minimum Distance', fit_reg=True,
                        line_kws={"color": "red"})
            sns.regplot(ax=axes[0, 2], data=df_scatter, y='Median of SNRs (dB)', x='Median of MSEs', logx=True,
                        line_kws={"color": "red"})
            ...
            sns.regplot(ax=axes[1, 0], data=df_scatter, y='Mean of MSEs', x='Minimum Distance', order=2, ci=None,
                        x_jitter=.05, line_kws={"color": "red"})  # order=3, ci=None, x_jitter=.05)
            sns.regplot(ax=axes[1, 1], data=df_scatter, y='Mean of SNRs (dB)', x='Minimum Distance', fit_reg=True,
                        line_kws={"color": "red"})
            sns.regplot(ax=axes[1, 2], data=df_scatter, y='Mean of SNRs (dB)', x='Mean of MSEs', logx=True,
                        line_kws={"color": "red"})  # order=3, ci=None, x_jitter=.05)
            ...
            sns.regplot(ax=axes[2, 0], data=df_scatter, x='Theor. MD (from SNR)', y='Minimum Distance', fit_reg=True,
                        line_kws={"color": "red"})
            sns.regplot(ax=axes[2, 1], data=df_scatter, y='Theor. MD (from SNR)', x='Minimum Distance', fit_reg=True,
                        line_kws={"color": "red"})
